...
- [ ] IIS에서 HTTPS 적용
- [ ] `CORS_ALLOWED_ORIGINS`를 실제 프론트엔드 도메인으로 제한 (옛 키 `CORS_ORIGINS` 는 backward-compat 폴백 — 새 키로 교체 권장)
- [ ] `MONIGRID_SECRET_KEY` 설정 — 알림 채널 자격증명 암호화 마스터 키, 미설정 시 `FLASK_ENV != development` 에서 부팅 실패
...
> **settings DB(`config` 섹션, 즉 최초 시드된 `config.json`)** 에서 관리합니다.
> `.env` 는 **보안 키 + 환경 분기 + 부트스트랩 폴백** 만 담당합니다.
> (옛 `LOG_*`, `API_*`, `THREAD_POOL_SIZE`, `CACHE_*`, `ENABLE_CACHE`,
> `DB_POOL_TIMEOUT_SEC`, `DB_POOL_RECYCLE_SEC`, `JWT_EXPIRATION_HOURS`,
> `DEBUG` 환경변수는 더 이상 코드에서 읽지 않으므로 제거되었습니다.)

### 13.1 보안 / 부트스트랩 (자주 설정)

| 변수 | 기본값 | 설명 |
|------|--------|------|
| `MONIGRID_SECRET_KEY` | (없음, dev fallback) | 알림 채널(SMTP 등) 자격증명 암호화 마스터 키. **`FLASK_ENV != development` 이면 미설정 시 부팅 실패** |
| `MONIGRID_SECRET_KEYS` | (없음) | 키 회전용 CSV (`신규,이전`). 첫 키가 active(암호화), 나머지는 복호화 전용. 설정 시 `MONIGRID_SECRET_KEY` 무시 |
| `JWT_SECRET_KEY` | (안전 기본값) | JWT 서명 키. 32B 미만이면 안전 기본값으로 폴백 + WARN 로그 — 운영에서 반드시 강한 무작위 값으로 교체 |
| `JWT_ALGORITHM` | `HS256` | JWT 알고리즘 |
| `AUTH_USERNAME` / `AUTH_PASSWORD` | `admin` / `admin` | 부트스트랩 admin. **`monigrid_users` 에 admin 역할 계정이 1개라도 있으면 자동 비활성화** |
| `ADMIN_USERNAME` | `admin` | 관리자 역할 판별 사용자명 (부트스트랩 로그인용) |
...
| `USE_WAITRESS` | `1` | `0` → Werkzeug, `1` → Waitress |
| `WAITRESS_THREADS` | `16` | Waitress 워커 스레드 수 |
| `CORS_ALLOWED_ORIGINS` | (wildcard) | 쉼표 구분 CORS allowlist. 미설정 + production 이면 wildcard `*` + WARN |
...
| 변수 | 기본값 | 설명 |
|------|--------|------|
| `DB_POOL_SIZE` | `5` | 모니터링 대상 DB 연결 풀 크기 |
| `DB_CONNECT_RETRY_ATTEMPTS` | `3` | 신규 connect 재시도 횟수 |
| `DB_CONNECT_RETRY_BACKOFF_SEC` | `0.5` | 재시도 backoff (초) |
| `DB_CONNECT_VALIDATION_TIMEOUT_SEC` | `2` | 풀 반환 시 validation 타임아웃 (초) |
| `SETTINGS_DB_RECONNECT_ATTEMPTS` | `3` | settings DB 재접속 시도 횟수 |
| `SETTINGS_DB_RECONNECT_BACKOFF_SEC` | `0.5` | settings DB 재접속 backoff (초) |
| `JAVA_HOME` | OS 기본 | JDK 11+ 경로 (JVM 자동 탐색이 실패할 때 명시) |
| `MONITORING_INIT_SETTINGS_PATH` | `(BASE_DIR/initsetting.json)` | `initsetting.json` 경로 오버라이드 |
| `MONITORING_CONFIG_PATH` | `(BASE_DIR/config.json)` | 최초 시드용 `config.json` 경로 오버라이드 |
| `TIMEMACHINE_DB_PATH` | `(BASE_DIR/timemachine.db)` | TimeMachine 로컬 SQLite 파일 경로 |
| `HEALTHCHECK_BLOCK_PRIVATE` | `1` (보안 우선) | `1` → health-check 프록시에서 사설/loopback/link-local IP 차단. DNS 호스트네임을 IP 로 재해석 후 검사. 클라우드 메타데이터 IP(`169.254.169.254`, `100.100.100.200`, `fd00:ec2::254`, `metadata.google.internal`) 는 설정과 무관하게 항상 차단. 내부 LAN 모니터링이 필요하면 `0` 으로 명시 설정 |

### 13.4 알림 서브시스템 (옵션 — default 사용 가능)
...
| `MONIGRID_NOTIFICATION_FAULT` | (없음) | `raise` / `hang` / `fail` — 5중 격리 검증용 (dev 전용) |

### 13.5 settings DB(config) 에서 관리되는 항목

아래는 `.env` 가 아니라 settings DB 의 `config` 섹션에서 변경합니다. 운영 중 변경 시 `POST /reload-config` 로 재로드합니다.

| 영역 | 키 | 비고 |
|------|----|------|
| 서버 | `server.host`, `server.port`, `server.thread_pool_size` | 옛 `API_HOST`/`API_PORT`/`THREAD_POOL_SIZE` 대체 |
| 로깅 | `logging.directory`, `logging.file_prefix`, `logging.retention_days`, `logging.slow_query_threshold_sec`, `logging.loglevel` | 옛 `LOG_*` 대체 |
| Rate limit | `rate_limits.*` | 옛 `API_RATE_LIMIT` 대체 |
...
| CORS 오류 | 백엔드 CORS 설정 미포함 | `CORS_ALLOWED_ORIGINS` 에 프론트엔드 도메인 추가 (옛 `CORS_ORIGINS` 는 backward-compat 폴백) |
