...
1. [시스템 구성](#1-시스템-구성)
2. [설치 및 배포](#2-설치-및-배포)
3. [config.json 설정](#3-configjson-설정)
4. [인증 및 보안](#4-인증-및-보안)
...
- 엔드포인트별 SQL 쿼리 조회/수정
- SQL 구문 하이라이팅
- 실시간 유효성 검사 (금지 구문, 오타 감지)
- 저장 시 SQL 파일 즉시 반영 및 캐시 갱신
...
| 오타 감지 | config.json `sql_validation.typo_patterns`에 정의된 패턴 |
...
| `GET` | `/dashboard/sql-editor/<api_id>` | SQL 조회 |
| `PUT` | `/dashboard/sql-editor/<api_id>` | SQL 수정 및 캐시 갱신 |
| `GET` | `/dashboard/sql-editor/files` | SQL 파일 목록 |
| `POST` | `/dashboard/sql-editor/files` | 새 SQL 파일 생성 |
...
- `config.json` 전체 내용을 JSON 텍스트 편집기로 조회/수정
- JSON 유효성 검사
- 저장 시 자동 핫 리로드
...
- DB 비밀번호 등 민감 정보가 포함되어 있으므로 주의해서 다루세요.
...
| 파일/디렉토리 | 중요도 | 설명 |
|-------------|--------|------|
| `config.json` | **필수** | 모든 연결/API/설정 |
| `sql/*.sql` | **필수** | SQL 쿼리 파일 |
