...
- **0.5 단위 위젯 크기** — 내부 그리드는 24컬럼이지만 사용자 입력은 12컬럼 기준 0.5 단위(`SIZE_UNIT_SCALE = 2`). 드래그 리사이즈 / 모달 직접 입력 모두 0.5 스텝으로 스냅됩니다. 이전 버전(12컬럼)에서 저장된 레이아웃은 첫 로드 시 `dashboardSettings.layoutScale` 플래그 기준으로 1회 자동 ×2 보정됩니다.
- **7종 위젯 타입** 지원:
  | 위젯 | 설명 |
  |------|------|
  ...
  | API 상태 리스트 | 백엔드 등록된 `http_status` 모니터 대상 다중 선택 → BE 가 주기 폴링한 결과를 한 화면에 표시 |
  | 네트워크 테스트 (Ping/Telnet) | 백엔드 등록된 `network` 대상 다중 선택 |
  | 서버 리소스 모니터링 | 백엔드 등록된 `server_resource` 대상 다중 선택 (CPU/Memory/Disk) |
- **위젯별 독립 리프레시 주기 설정** — 1~3600초
- **LIVE / SLOW-LIVE / DEAD 상태 표시** — 응답 지연 시 노란색 경고
- **폭 기반 반응형 표시** — 서버 리소스 / API 상태 리스트 / 네트워크 테스트 카드는 위젯 폭(user unit)에 따라 mini → compact → normal → wide 4단계로 폰트·gap·라벨 표시 여부를 자동 조절
...
- **데이터 API 위젯** (테이블 / 차트 / 단일 헬스체크) — 엔드포인트 URL 직접 입력
- **서버 리소스 / 네트워크 체크 / API 상태 리스트 위젯** — 호스트·URL·자격증명을 위젯에 입력하지 않고, 백엔드 설정에 등록된 모니터 대상 중에서 **체크박스 다중 선택** (`MonitorTargetPicker` 공용 컴포넌트). 추가 모달 / ⚙ 설정 모달 양쪽에서 동일 픽커 사용. API 상태 리스트는 v2.3+ 부터 BE 컬렉터 기반(`http_status` 타겟)으로 전환되었으며, 기존 URL 입력 위젯은 자동 마이그레이션되지 않으므로 새 카드로 다시 등록해야 합니다.

### 모달 / 카드 구조

- **공용 위젯 설정 모달** (`WidgetSettingsModal`) — 모든 위젯의 ⚙ 설정 모달이 단일 wrapper 를 공유합니다. Esc 닫기 / body scroll lock / `role=dialog` / 첫 입력 자동 focus / 오버레이 클릭 닫기 (`closeOnBackdropClick` 으로 opt-out) 가 일괄 적용됩니다.
- **차트 카드 분해** — `BarChartCard` / `LineChartCard` 는 (1) 헤더+조합, (2) `*SettingsModal`, (3) `*Body` (recharts 본체), (4) `use*Settings` 훅 4파일로 분해되어 있습니다. 메인 카드 파일이 5~6배 작아져 헤더/주요 동작을 한 화면에서 볼 수 있습니다.
- **CSS 토큰화** — `styles/index.css` 의 `:root` 에 카드 표면 토큰(`--card-bg`, `--card-header-bg`, `--card-border`, `--card-shadow`, `--card-blur`, `--card-radius` 등) 이 정의되어 있어 브랜드 색상 변경이 한 파일에서 가능합니다.
...
│   │   ├── ApiCardSettingsModal.jsx # 데이터 테이블 위젯 설정 모달
│   │   ├── ApiCardRowDetailModal.jsx # 행 상세 팝업
│   │   ├── apiCardHelpers.js        # 데이터 테이블 정규화 헬퍼
│   │   ├── HealthCheckCard.jsx      # 웹서버 상태 체크 위젯
│   │   ├── LineChartCard.jsx        # 라인차트 위젯 (헤더 + 조합)
│   │   ├── LineChartBody.jsx        # 라인차트 본체 (recharts)
│   │   ├── LineChartSettingsModal.jsx # 라인차트 설정 모달
│   │   ├── BarChartCard.jsx         # 바차트 위젯 (헤더 + 조합)
│   │   ├── BarChartBody.jsx         # 바차트 본체 (recharts)
│   │   ├── BarChartSettingsModal.jsx # 바차트 설정 모달
│   │   ├── StatusListCard.jsx       # API 상태 리스트 위젯 (BE `http_status` 타겟 픽커 사용)
...
│   │   ├── ServerRow.jsx            # 서버 리소스 한 행 (CPU/MEM/DISK 메트릭)
│   │   ├── serverResourceHelpers.js # 서버 리소스 표시 헬퍼 (displayMode 등)
│   │   ├── ServerDetailPopup.jsx    # 서버 상세 팝업 (CPU/MEM/DISK 실시간 차트)
...
│   │   ├── WidgetSettingsModal.jsx  # 모든 위젯 설정 모달의 공용 wrapper (Esc/scroll-lock/role=dialog) ★
│   │   ├── widgetUtils.js           # 위젯 공통 헬퍼 (clamp, toGridSize, toUserSize, formatInterval, normalizeChartData)
│   │   ├── icons.jsx                # Lucide 스타일 인라인 SVG 아이콘 모음
│   │   ├── IncidentTimelineCard.jsx # 장애 타임라인
│   │   └── *.css                    # 컴포넌트별 스타일
│   ├── hooks/
│   │   ├── useApi.js                # 위젯별 API 폴링 통합 훅 (배럴 export)
│   │   ├── useApiData.js            # 단일 API 데이터 훅 (epoch ref 기반 stale-response guard)
│   │   ├── useMultipleApiData.js    # 다중 API 데이터 훅
│   │   ├── useWidgetApiData.js      # 위젯 API 데이터 매니저 (per-endpoint 429 격리)
│   │   ├── useBarChartSettings.js   # BarChart 상태/파생/핸들러 ★
│   │   └── useLineChartSettings.js  # LineChart 상태/파생/핸들러 ★
│   ├── pages/
...
각 위젯의 헤더에서 **`⚙`** 버튼을 클릭하면 설정 팝업이 열립니다. 모든 위젯의 설정 모달은 공용 `WidgetSettingsModal` wrapper 를 사용하므로 동작이 동일합니다 — Esc 로 닫기 / 모달 열린 동안 body scroll lock / 첫 입력 자동 focus / 오버레이 클릭 닫기 (네트워크 테스트처럼 입력 draft 가 많은 카드는 `closeOnBackdropClick=false` 로 보호).

| 설정 항목 | 설명 |
|-----------|------|
| 제목 | 위젯 이름 변경 |
| 위젯 크기 | 가로(W) / 세로(H) — **0.5 단위** 입력. 사용자 단위 1 = 그리드 셀 2개로 저장됨. 가로 1~12, 세로 2~12 |
...
여러 URL 의 HTTP 200 상태를 한눈에 확인합니다. v2.3+ 부터 BE 컬렉터 기반(`monigrid_monitor_targets` 의 `http_status` 타입) 으로 전환되었습니다.

**설정:**
- 백엔드 설정(`⚙`) → **API 상태** 탭에서 관리자가 `label` / `url` / `interval_sec` / `timeout_sec` 으로 대상을 등록 (또는 `MonitorTargetsTab` 의 `http_status` 카드)
- 위젯 ⚙ 에서 등록된 대상 중 **체크박스 다중 선택**
- BE 가 각 대상을 `interval_sec` 주기로 호출 → 인메모리 스냅샷에 저장하므로, 같은 URL 을 보는 브라우저가 100개여도 외부 호출은 BE 가 1회만 수행합니다
- **반응형 표시** — 위젯 폭(`w`) 에 따라 compact(≤3 user-unit) / normal / wide 단계로 row 높이·폰트·URL 컬럼 표시 여부가 달라집니다. compact 모드에서는 행 hover 시 title 툴팁으로 숨겨진 URL/오류가 표시됩니다.

> 기존(v2.2 이전) URL 입력 위젯은 자동 마이그레이션되지 않습니다. 위젯을 다시 추가하거나 ⚙ → 대상 선택으로 재구성하세요.
...
브라우저에서 프론트엔드와 백엔드의 origin 이 다른 경우 백엔드의 CORS 설정이 필요합니다.
...
> IIS 단일 사이트 배포처럼 같은 origin 으로 서빙하면 CORS 설정 없이도 동작합니다.

---

## 12. 로컬 스토리지

대시보드는 다음 정보를 브라우저 로컬 스토리지에 자동 저장합니다 (정확한 키 이름은 `src/store/storageKeys.js` 참조). 단, 위젯 레이아웃·임계값·알람 소리 등 사용자 환경설정은 BE (`monigrid_user_preferences`) 에 우선 저장되며 localStorage 는 오프라인 캐시 및 첫 렌더 시드 용도로만 쓰입니다.

| 키 | 내용 |
|----|------|
| `auth_token` | JWT 토큰 (로그인 유지) |
| `user` | 로그인 사용자 정보 |
| `dashboard_widgets` | 위젯 목록 전체 (타입, 설정, targetIds 등) |
| `dashboard_layouts` | 위젯별 위치 및 크기 (24-col 그리드 기준) |
| `dashboard_settings` | 전역 설정 (폰트 크기, `layoutScale` 등) |
| `alarm_sound` | 선택된 알람 경고음 |
| `alarm_sound_enabled` | 경고음 활성화 여부 |
| `api_base_url` | 저장된 API 서버 URL |

> 401 응답을 받으면 `auth_token` / `user` 와 대시보드 캐시가 함께 정리되고, dashboardStore 는 자동으로 server sync 를 비활성화합니다 (`http.js` 의 401 인터셉터).
...
| 의존성 충돌 | `node_modules` 와 `package-lock.json` 삭제 후 `npm install` 재실행 |
